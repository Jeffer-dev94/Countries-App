<template>
  <div class="block w-full my-10">
    <ul class="flex flex-row items-center justify-center w-full gap-4">
      <li>
        <button
          @click="handleOnClick(currentPage - 1)"
        ...
        >
          <ChevronLeftIcon
            class="size-4 text-current group-hover:-translate-x-0.5 transition-transform duration-500"
          />
      ...
      <li class="flex gap-2">
        <button
          v-if="currentPage > 2 && pageRange < totalPages"
          @click="handleOnClick(backNumber)"
        ...
        <div class="flex gap-2">
          <button
            v-for="page in range(startPage, lastPage)"
            :key="page"
            :class="[
        ...
        <button
          v-if="currentPage + pageRange < totalPages"
          @click="handleOnClick(forwardNumber)"
          class="text-app-blue-800 dark:text-white"
        >
          ...
        </button>
