<template>
  <Listbox v-model="selectedOption" @update:modelValue="value => emit('update:modelValue', value)">
    <div class="relative w-full">
      <ListboxButton
        ...
        <span class="block truncate">{{ selectedOption?.name ?? placeholder }}</span>
        ...
        <ListboxOptions
          ...
          <ListboxOption v-slot="{ active, selected }" v-for="item in items" :key="item.name" :value="item"
            ...
            <li :class="[
              ...
              <span v-if="selected" class="absolute inset-y-0 left-0 flex items-center pl-2 text-current">
